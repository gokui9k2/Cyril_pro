...
def create_sequences(data, seq_length, pred_length, target_col_name):
    """
    Creates Input X and Target y sequences using a sliding window approach
    """
    X, y = [], []
    ...
    # Ensure y is 2D (batch_size, pred_length) even if pred_length is 1
    if y.ndim == 1:
        y = y.reshape(-1, 1)
...
def create_loaders(data, seq_length, pred_length, batch_size=32, target_col_name='target'):
    """
    Splits data into Train/Val/Test and wraps them in PyTorch DataLoaders
    """
    total_len = len(data)
    ...
    # Slice the dataframe physically to avoid data leakage
...
def create_mae_masked_sequences(data, seq_length=96, patch_size=6, num_masked_patches=4, mask_value=0.0):
    """
    Prepares data for Masked Autoencoders BERT style pre training for Time Series
    """
...
def create_loaders_mae(train_scaled, val_scaled, test_scaled, seq_length=96, patch_size=6, num_masked_patches=4, batch_size=64, num_workers=0):
    """
    Wraps the MAE sequence creator into PyTorch DataLoaders the Dataset yields tuples of Masked_Input, Binary_Mask, Original_Target
    """
...
def create_sequences_direct(data, seq_length, pred_length, target_col_name):
    X_encoder, y_target = [], [] 
    ...
    return X_encoder, y_target

def create_loaders_direct(train_scaled, val_scaled, test_scaled, seq_length, pred_length, target_col_name, batch_size=32, num_workers=0):

    X_enc_train, y_train = create_sequences_direct(train_scaled, seq_length, pred_length, target_col_name)
    X_enc_val, y_val = create_sequences_direct(val_scaled, seq_length, pred_length, target_col_name)
    X_enc_test, y_test = create_sequences_direct(test_scaled, seq_length, pred_length, target_col_name)

    train_dataset = TensorDataset(torch.FloatTensor(X_enc_train), torch.FloatTensor(y_train))
    val_dataset = TensorDataset(torch.FloatTensor(X_enc_val), torch.FloatTensor(y_val))
    test_dataset = TensorDataset(torch.FloatTensor(X_enc_test), torch.FloatTensor(y_test))
